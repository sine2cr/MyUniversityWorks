...
datalist = []#储存总体数据
def main():
    url='http://www.hnloudi.gov.cn/loudi/0601/list.shtml'
    getData(url)
    for i in range(2, 51):
        url = 'http://www.hnloudi.gov.cn/loudi/0601/list_{}.shtml'.format(i)
        getData(url)
    saveData(datalist)
def askURL(url):
    head={
...
def getData(url):
    ...
    for item in soup.select('.mainContent-pad > ul > li'):  # http://www.hnloudi.gov.cn/loudi/0601/202207/4fc4bfba62cf410689fd007293954192.shtml
        data = []
        item = str(item)
        item = item.replace('\n', '')
        ...
        if '2022' not in time[0]:
            break
        data.append('http://www.hnloudi.gov.cn' + link[0])
        data.append(news[0])
        if time:
            data.append(time[0])
        else:
