...
datalist = []#储存总体数据
def main():
    url='http://www.zjj.gov.cn/c32/index.html'
    getData(url)
    for i in range(2, 31):
        url = 'http://www.zjj.gov.cn/c32/pages/{}.html'.format(i)
        getData(url)
    saveData(datalist)
...
def getData(url):
    ...
    for item in soup.select('.list_con_r > ul > li'):  # http://yueyang.gov.cn/zwdt/content_1966551.html
        data = []
        item = str(item)
        if 'line' not in item:
            item = item.replace('\n', '')
