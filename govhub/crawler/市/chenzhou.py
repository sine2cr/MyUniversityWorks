...
datalist = []#储存总体数据
def main():
    url='http://www.czs.gov.cn/html/dtxx/zwdt/zwyw/default.htm'
    getData(url)
    for i in range(1, 38):
        url = 'http://www.czs.gov.cn/html/dtxx/zwdt/zwyw/default.jsp?jsp.offset={}'.format(i*15)
        getData(url)
    saveData(datalist)
def askURL(url):
    head={
...
def getData(url):
    ...
    for item in soup.select('.yaowennr > ul > li'):  # http://www.czs.gov.cn/html/dtxx/zwdt/zwyw/content_3468919.html
        data = []
        item = str(item)
        item = item.replace('\n', '')
        ...
        if '2022' not in time[0]:
            break
        data.append('http://www.czs.gov.cn/html/dtxx/zwdt/zwyw/' + link[0])
        data.append(news[0])
        if time:
            data.append(time[0])
        else:
