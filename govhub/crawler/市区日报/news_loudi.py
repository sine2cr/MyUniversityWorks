...
def main():
    url='http://ldrb.ldnews.cn/index.html'
...
def askURL(url):
    ...
    br.get(url)
    time.sleep(1)
    today_url = br.find_element(By.XPATH,'//div[@id="papers"]/ul/li/a').get_attribute('href')#获取每天最新的报刊
    br.get(today_url)
    try:
        html = []
        ...
        while(1):
            try:
                if "paper_next" in br.page_source:#翻页
                    right = br.find_element(By.XPATH, '//div[@class="paper_pn"]/div[@class="paper_next"]/a')
                    right.click()
                    time.sleep(1)
                    html.append(br.page_source)
                else:
...
def getData(url):
    ...
    for html in html:
        ...
        for item in soup.select('div.banmian_lt > ul#bm_biaoti > li'):  # http://yn.gov.cn/ywdt/ynyw/202206/t20220625_243614.html
            data = []
            item = str(item)
            link = re.findall(findlink, item)  # 链接 http://ldrb.ldnews.cn/2022/08/16/613284.html
            news = re.findall(findnews, item)  # 题目
            news[0]=news[0].replace('“','')
            #time = re.findall(findtime, item)  # 时间
            data.append('http://ldrb.ldnews.cn' + link[0])
            data.append(news[0])
