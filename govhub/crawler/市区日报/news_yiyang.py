...
findlink=re.compile(r'<a href="(.*?)" t')
findnews=re.compile(r'title="(.*?)">')
findtime=re.compile(r'(\d*-\d*-\d*)')
def main():
    url='https://epaper.yyrb.cn/'
...
def askURL(url):
    ...
    br.get(url)
    time.sleep(1)
    today_url = br.find_element(By.XPATH,'//div[@id="container"]/div/a').get_attribute('href')#获取每天最新的报刊
    br.get(today_url)
    try:
        html = []
        ...
        while(1):
            try:
                if 'class="tool_btn_li xb ">下一版' in br.page_source:#翻页
                    right = br.find_element(By.XPATH, '//div[@id="index_tool"]/ul[@class="tool_btns"]/li/a[contains(text(),"下一版")]')
                    right.click()
                    time.sleep(1)
                    html.append(br.page_source)
                else:
...
def getData(url):
    ...
    for html in html:
        ...
        for item in soup.select('div.title_list > ul > li > h3 > a'):
            data = []
            item = str(item)
