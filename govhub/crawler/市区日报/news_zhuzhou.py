        ...
        while(1):
            try:
                if "下一版" in br.page_source:#翻页
                    right = br.find_element(By.XPATH, '//div[@class="pull-right"]/span/a[@class="nextArt"]')
                    right.click()
                    time.sleep(1)
                    html.append(br.page_source)
                else:
...
def getData(url):
    ...
    for html in html:
        ...
        for item in soup.select('div.newsNav > ul.newsList > li.clearfix > a'):
            data = []
            item = str(item)
            link = re.findall(findlink, item)  # 链接 https://zzrb.zznews.gov.cn/content/202208/18/c1595529.html
            news = re.findall(findnews, item)  # 题目
            if news[0] != ' ':
                news[0]=news[0].replace('“','')
                #time = re.findall(findtime, item)  # 时间
                data.append('https://zzrb.zznews.gov.cn/'+link[0])
                data.append(news[0])
