...
def main():
    url='http://paper.zjjnews.cn:8081/'
...
def getData(url):
    ...
    for html in html:
        ...
        for item in soup.select('div.news-list > ul > li.resultList >a'):  # http://yn.gov.cn/ywdt/ynyw/202206/t20220625_243614.html
            data = []
            item = str(item)
            link = re.findall(findlink, item)  # 链接 http://paper.zjjnews.cn:8081/zjjrbpc/content/202208/16/content_60057.html
            news = re.findall(findnews, item)  # 题目
            if news[0] != ' ':
                news[0]=news[0].replace('“','')
                data.append('http://paper.zjjnews.cn:8081/zjjrbpc'+link[0])
                data.append(news[0])
                data.append(time[0])
