...
def askURL(url):
    ...
    base_url=base_url.replace('Qpaper.html','')
    try:
        html = []
        html.append(br.page_source)
        while(1):
            try:
                if ("下一版" in br.page_source)and("goVer('-1')" not in br.page_source):#翻页
                    right = br.find_element(By.XPATH, '//div[@class="pmpc"]/div[@id="map_buttons"]/a[contains(text(),"下一版")]')
                    right.click()
                    time.sleep(1)
                    html.append(br.page_source)
                else:
...
def getData(url):
    ...
    for html in html:
        ...
        for item in soup.select('div#doclist > ul > li > a'):
            data = []
            item = str(item)
            link = re.findall(findlink, item)  # 链接 http://paper.0746news.com/Html/2022-08-18/213502.html
