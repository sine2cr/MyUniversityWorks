...
def askURL(url):
    ...
    base_url=base_url.replace('node_1.htm?v=1','')
    try:
        html = []
        ...
        while(1):
            try:
                if "下一版" in br.page_source:#翻页
                    right = br.find_element(By.XPATH, '//tr/td/a[@id="ed_next"]')
                    right.click()
                    time.sleep(1)
                    html.append(br.page_source)
                else:
...
def getData(url):
    ...
    for html in html:
        ...
        for item in soup.select('div#main-ed-articlenav-list > table > tbody > tr >td >div'):
            data = []
            item = str(item)
            link = re.findall(findlink, item)  # 链接 http://epaper.hyqss.cn/html/2022-08/17/content_19621_6605269.htm
